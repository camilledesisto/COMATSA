...
ef <- ef %>%rename(DEM = Band.1.1, FL = ForestLoss, FH = Layer_1, TRI =Band.1.2) %>%mutate(pred = coefficient_data[[5]]*(TRI) + (coefficient_data[[3]]*(DEM)) + (coefficient_data[[2]]*(FH)) + (coefficient_data[[4]]*FL) + coefficient_data[[1]])
...
cheiro_spatial_predict@data@values[1] <- 1400

plot_cheiro <- ggplot() +
  layer_spatial(cheiro_spatial_predict)+
  theme_void()+
...
micro_spatial_predict$pred@data@values[1] <- 1400

plot_micro <- ggplot() +
  layer_spatial(micro_spatial_predict)+
  theme_void()+
 scale_fill_gradientn(colours = terrain.colors(10), name = expression("Individuals/Km"^2), trans="reverse",na.value="white")+
 layer_spatial(bound, fill=NA, color="black")

allo_spatial_predict$pred@data@values[1] <- 1400

plot_allo <- ggplot() +
  layer_spatial(allo_spatial_predict)+
  theme_void()+
 scale_fill_gradientn(colours = terrain.colors(10), name = expression("Individuals/Km"^2), trans="reverse",na.value="white")+
 layer_spatial(bound, fill=NA, color="black")

lepi_spatial_predict$pred@data@values[1] <- 1400

plot_lepi <- ggplot() +
  layer_spatial(lepi_spatial_predict)+
  theme_void()+
 scale_fill_gradientn(colours = terrain.colors(10), name = expression("Individuals/Km"^2), trans="reverse",na.value="white")+
 layer_spatial(bound, fill=NA, color="black")

avahi_spatial_predict$pred@data@values[1] <- 1400

plot_avahi <- ggplot() +
  layer_spatial(avahi_spatial_predict)+
  theme_void()+
 scale_fill_gradientn(colours = terrain.colors(10), name = expression("Individuals/Km"^2), trans="reverse",na.value="white")+
 layer_spatial(bound, fill=NA, color="black")

prop_spatial_predict$pred@data@values[1] <- 1400

plot_prop <- ggplot() +
  layer_spatial(prop_spatial_predict)+
  theme_void()+
 scale_fill_gradientn(colours = terrain.colors(10), name = expression("Individuals/Km"^2), trans="reverse",na.value="white")+
 layer_spatial(bound, fill=NA, color="black")

rubri_spatial_predict$pred@data@values[1] <- 1400

plot_rubri <- ggplot() +
  layer_spatial(rubri_spatial_predict)+
  theme_void()+
 scale_fill_gradientn(colours = terrain.colors(10), name = expression("Individuals/Km"^2), trans="reverse",na.value="white")+
 layer_spatial(bound, fill=NA, color="black")
...
plot_albi <- ggplot() +
  layer_spatial(albi_spatial_predict)+
  theme_void()+
 scale_fill_gradientn(colours = terrain.colors(10), name = expression("Individuals/Km"^2), trans="reverse",na.value="white")+
 layer_spatial(bound, fill=NA, color="black")

hapa_spatial_predict$pred@data@values[1] <- 1400

plot_hapa <- ggplot() +
  layer_spatial(hapa_spatial_predict)+
  theme_void()+
 scale_fill_gradientn(colours = terrain.colors(10), name = expression("Individuals/Km"^2), trans="reverse",na.value="white")+
 layer_spatial(bound, fill=NA, color="black")

plot_density_predict <- ggarrange(plot_cheiro, plot_micro, plot_allo, plot_lepi, plot_avahi, plot_prop, plot_rubri, plot_albi, plot_hapa, common.legend = TRUE, labels = c("C. crossleyi", "M. lehilahytsara", "A. trichotis", "L. saeli", "A. laniger", "P. candidus", "E. rubriventer", "E. albifrons", "H. occidentalis"))
...
plot <- ggplot() +
  layer_spatial(pred$pred)+
  theme_void()+
 scale_fill_gradientn(colours = terrain.colors(10), name = expression("Density/Km"^2), trans="reverse",na.value="white")+
 # scale_fill_continuous(type = "viridis",na.value="white")+
  #guides(fill=guide_legend(title=expression("Density/Km"^2)))+
 layer_spatial(bound, fill=NA, color="black")
