...
survey_all <- read.csv("~/Documents/GitHub/COMATSA_GitHub/COMATSA_GitHub/Data/lemurs_primary_in_transect_jan22.csv")
survey_all$Date <- mdy(survey_all$Date)
survey_all <- survey_all[survey_all$Date >= ymd("20220601"),]
...
#NREP NOCTURNAL
nrep_nocturnal <- survey_all %>% select(Site, Transect, Repetition, Type)%>%filter(Type=="Nocturnal") %>%drop_na(Transect) %>% group_by(Site, Transect) %>% mutate(NRep = max(Repetition)) %>% select(-Repetition)%>% distinct()
...
#NREP DIURNAL
nrep_diurnal <- survey_all %>% select(Site, Transect, Repetition, Type)%>%filter(Type=="Diurnal") %>%drop_na(Transect) %>% group_by(Site, Transect) %>% mutate(NRep = max(Repetition)) %>% select(-Repetition)%>% distinct()
...
survey_diurnal$Site2 <- survey_diurnal$Site
survey_diurnal$Transect2 <- survey_diurnal$Transect
survey_diurnal <-survey_diurnal %>% unite("Site_Transect", Site2:Transect2, sep="_")


#NOBS 

#not working, excluding many sites/transects
nobs <- survey_all %>% select(Site, Transect, Nom_sientifique)%>%drop_na(Nom_sientifique) %>%group_by(Site, Transect, Nom_sientifique) %>% mutate(NObs =n())%>% distinct()

nobs <-nobs %>% unite("Site_Transect", Site:Transect, sep="_")

#CHEIRO
nrep_cheiro <- survey_all %>%filter(Mois==1|Mois==2|Mois==3|Mois==11|Mois==12) %>%filter(Type=="Nocturnal") %>%select(Site, Transect, Repetition)%>%drop_na(Transect) %>% group_by(Site, Transect) %>% mutate(NRep = max(Repetition)) %>% select(-Repetition)%>% distinct()
...
#EFFORT
survey_nocturnal <- left_join(survey_nocturnal, transect_lengths, by =c("Site", "Transect"))
survey_nocturnal <- left_join(survey_nocturnal, nrep_nocturnal, by =c("Site", "Transect"))
survey_nocturnal$Effort <- survey_nocturnal$Length* survey_nocturnal$NRep
  
survey_diurnal <- left_join(survey_diurnal, transect_lengths, by =c("Site", "Transect"))
survey_diurnal <- left_join(survey_diurnal, nrep_diurnal, by =c("Site", "Transect"))
survey_diurnal$Effort <- survey_diurnal$Length* survey_diurnal$NRep
```

```{r Allocebus density calculations}

#Get data frame for just allocebus
allocebus_df <- survey_nocturnal  %>% filter(Nom_sientifique=="Allocebus_trichotis") %>% filter(Site =="Ambodivoara"|Site=="AmbodivohitraKobahina"| Site=="Antanambe"| Site=="Ambavala") %>% select(Nom_sientifique, Site, Transect, Angle, Dis, Effort, Site_Transect)
...
#Clean data frame
allocebus_df2 <- allocebus_df %>% select(Site_Transect,Effort, distance)
allocebus_df3 <-allocebus_df2

#Further clean data data set
allocebus_df3 <- allocebus_df3[!is.na(allocebus_df3$distance),]
allocebus_df3 <- allocebus_df3[(allocebus_df3$Site_Transect!="HT"),]
allocebus_df3$Area <- 0.02 #For now, assume an area of 1 
...
gof_ds(model2_allocebus)#significant, poor fit
...
AIC(model2_allocebus, model8_allocebus)
...
microcebus_df <- survey_nocturnal  %>% filter(Nom_sientifique=="Microcebus_lehilahytsara") %>% filter(Site =="Ambodivoara"|Site=="AmbodivohitraKobahina"| Site=="Antanambe"| Site=="Ambavala") %>% select(Nom_sientifique, Site, Transect, Angle, Dis, Effort, Site_Transect)
...
#Clean data frame
microcebus_df2 <- microcebus_df %>% select(Site_Transect,Effort, distance)
microcebus_df3 <-microcebus_df2

#Further clean data data set
microcebus_df3 <- microcebus_df3[!is.na(microcebus_df3$distance),]
microcebus_df3 <- microcebus_df3[(microcebus_df3$Site_Transect!="HT"),]
microcebus_df3$Area <- 0.2 #For now, assume an area of 1 
...
#Get detection probability
microcebus_detection_prob <- predict(model5_microcebus)$fitted[[1]]
...
cheirogaleus_df <- survey_nocturnal  %>% filter(Nom_sientifique=="Cheirogaleus_crossleyi") %>% filter(Site =="Ambodivoara"|Site=="AmbodivohitraKobahina"| Site=="Antanambe"| Site=="Ambavala") %>% select(Nom_sientifique, Site, Transect, Angle, Dis, Length, Site_Transect)
...
cheirogaleus_df <- cheirogaleus_df %>% unite("Site_Transect", Site:Transect, sep="_")

#Clean data frame
cheirogaleus_df2 <- cheirogaleus_df %>% select(Site_Transect,Effort, distance)
...
cheirogaleus_df3 <- cheirogaleus_df3[(cheirogaleus_df3$Site_Transect!="HT"),]
...
#Upload covariate data 
cheirogaleus_dat <- read.csv("~/Documents/GitHub/COMATSA_GitHub/COMATSA_GitHub/Data/cheiro_reps.csv")

cheirogaleus_dat <- left_join(cheirogaleus_dat, nrep_cheiro, by =c("Site_Transect"))
cheirogaleus_dat <- left_join(cheirogaleus_dat, nobs_cheiro, by =c("Site_Transect"))
cheirogaleus_dat <- cheirogaleus_dat %>% select(Site_Transect, NObs, NRep)
cheirogaleus_dat$NObs[is.na(cheirogaleus_dat$NObs)] <- 0
...
lepilemur_df <- survey_nocturnal  %>% filter(Nom_sientifique=="Lepilemur_seali") %>% filter(Site=="Ambodivoara"|Site=="AmbodivohitraKobahina"| Site=="Antanambe"| Site=="Ambavala") %>% select(Nom_sientifique, Site, Transect, Angle, Dis, Effort, Site_Transect)
...
#Clean data frame
lepilemur_df2 <- lepilemur_df %>% select(Site_Transect,Effort, distance)
...
lepilemur_df3 <- lepilemur_df3[(lepilemur_df3$Site_Transect!="HT"),]
...
gof_ds(model8_lepilemur)#significant,
gof_ds(model9_lepilemur)#significant,

#Of the models without poor fit, compare AIC calues
AIC(model1_lepilemur, model2_lepilemur, model3_lepilemur, model4_lepilemur,model5_lepilemur, model6_lepilemur)
...
avahi_df <- survey_nocturnal  %>% filter(Nom_sientifique=="Avahi_laniger") %>% filter(Site =="Ambodivoara"|Site=="AmbodivohitraKobahina"| Site=="Antanambe"| Site=="Ambavala") %>% select(Nom_sientifique, Site, Transect, Angle, Dis, Effort, Site_Transect)
...
#Clean data frame
avahi_df2 <- avahi_df %>% select(Site_Transect,Effort, distance)
avahi_df3 <-avahi_df2
...
avahi_df3 <- avahi_df3[(avahi_df3$Site_Transect!="HT"),]
...
#Upload covariate data 
avahi_dat <- read.csv("~/Documents/GitHub/COMATSA_GitHub/COMATSA_GitHub/Data/avahi_reps.csv")

avahi_dat <- left_join(avahi_dat, nrep_nocturnal, by =c("Site_Transect"))
avahi_dat <- left_join(avahi_dat, nobs[nobs$Nom_sientifique=="Avahi_laniger",], by =c("Site_Transect"))
avahi_dat <- avahi_dat %>% select(Site_Transect, NObs, NRep)
avahi_dat$NObs[is.na(avahi_dat$NObs)] <- 0
...
rubriventer_df <- survey_diurnal  %>% filter(Nom_sientifique=="Eulemur_rubriventer") %>% filter(Site =="Ambodivoara"|Site=="AmbodivohitraKobahina"| Site=="Antanambe"| Site=="Ambavala") %>% select(Nom_sientifique, Site, Transect, Angle, Dis, Effort, Site_Transect)
...
#Clean data frame
rubriventer_df2 <- rubriventer_df %>% select(Site_Transect,Effort, distance)
...
rubriventer_df3 <- rubriventer_df3[(rubriventer_df3$Site_Transect!="HT"),]
...
#Upload covariate data 
rubri_dat <- read.csv("~/Documents/GitHub/COMATSA_GitHub/COMATSA_GitHub/Data/rubri_reps.csv")

rubri_dat <- left_join(rubri_dat, nrep_diurnal, by =c("Site_Transect"))
rubri_dat <- left_join(rubri_dat, nobs[nobs$Nom_sientifique=="Eulemur_rubriventer",], by =c("Site_Transect"))
rubri_dat <- rubri_dat %>% select(Site_Transect, NObs, NRep)
rubri_dat$NObs[is.na(rubri_dat$NObs)] <- 0
...
albifrons_df <- survey_diurnal  %>% filter(Nom_sientifique=="Eulemur_albifrons") %>% filter(Site =="Ambodivoara"|Site=="AmbodivohitraKobahina"| Site=="Antanambe"| Site=="Ambavala") %>% select(Nom_sientifique, Site, Transect, Angle, Dis, Effort, Site_Transect)
...
#Clean data frame
albifrons_df2 <- albifrons_df %>% select(Site_Transect,Effort, distance)
...
albifrons_df3 <- albifrons_df3[(albifrons_df3$Site_Transect!="HT"),]
...
#Upload covariate data 
albi_dat <- read.csv("~/Documents/GitHub/COMATSA_GitHub/COMATSA_GitHub/Data/albi_reps.csv")

albi_dat <- left_join(albi_dat, nrep_diurnal, by =c("Site_Transect"))
albi_dat <- left_join(albi_dat, nobs[nobs$Nom_sientifique=="Eulemur_albifrons",], by =c("Site_Transect"))
albi_dat <- albi_dat %>% select(Site_Transect, NObs, NRep)
albi_dat$NObs[is.na(albi_dat$NObs)] <- 0
...
propithecus_df <- survey_diurnal  %>% filter(Nom_sientifique=="Propithecus_candidus") %>% filter(Site =="Ambodivoara"|Site=="AmbodivohitraKobahina"| Site=="Antanambe"| Site=="Ambavala") %>% select(Nom_sientifique, Site, Transect, Angle, Dis, Effort, Site_Transect)
...
#Clean data frame
propithecus_df2 <- propithecus_df %>% select(Site_Transect,Effort, distance)
...
propithecus_df3 <- propithecus_df3[(propithecus_df3$Site_Transect!="HT"),]
...
#Upload covariate data 
prop_dat <- read.csv("~/Documents/GitHub/COMATSA_GitHub/COMATSA_GitHub/Data/prop_reps.csv")

prop_dat <- left_join(prop_dat, nrep_diurnal, by =c("Site_Transect"))
prop_dat <- left_join(prop_dat, nobs[nobs$Nom_sientifique=="Propithecus_candidus",], by =c("Site_Transect"))
prop_dat <- prop_dat %>% select(Site_Transect, NObs, NRep)
prop_dat$NObs[is.na(prop_dat$NObs)] <- 0
...
hapalemur_df <- survey_diurnal  %>% filter(Nom_sientifique=="Hapalemur_occidentalis") %>% filter(Site =="Ambodivoara"|Site=="AmbodivohitraKobahina"| Site=="Antanambe"| Site=="Ambavala") %>% select(Nom_sientifique, Site, Transect, Angle, Dis, Effort, Site_Transect)
...
#Clean data frame
hapalemur_df2 <- hapalemur_df %>% select(Site_Transect,Effort, distance)
...
hapalemur_df3 <- hapalemur_df3[(hapalemur_df3$Site_Transect!="HT"),]
...
#Get detection probability
hapalemur_detection_prob <- predict(model7_hapalemur)$fitted[[1]] #odd that this is 1? Assumed perfect detection but they are so cryptic! 

#Calculate strp width (2 * maximum distance, of the truncated distance data)
hapalemur_strip_width <- max(hapalemur_df3$distance) * 2

#Upload covariate data 
hapa_dat <- read.csv("~/Documents/GitHub/COMATSA_GitHub/COMATSA_GitHub/Data/hapa_reps.csv")
hapa_dat <- left_join(hapa_dat, nrep_diurnal, by =c("Site_Transect"))
hapa_dat <- left_join(hapa_dat, nobs[nobs$Nom_sientifique=="Hapalemur_occidentalis",], by =c("Site_Transect"))
hapa_dat <- hapa_dat %>% select(Site_Transect, NObs, NRep)
hapa_dat$NObs[is.na(hapa_dat$NObs)] <- 0
