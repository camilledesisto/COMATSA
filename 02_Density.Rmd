...
survey_all <- read.csv("~/Documents/GitHub/COMATSA_GitHub/COMATSA_GitHub/Data/lemurs_primary_in_transect_feb2.csv")
survey_all$Type[survey_all$Type=="diurnal"] <- "Diurnal"

#remove detections not based on sight
survey_all <- survey_all %>% filter(ObjectID != 1440 & ObjectID != 1418 & ObjectID != 417 & ObjectID != 418)
...
nrep_nocturnal_distance <- survey_all %>% select(Site, Transect, Repetition, Type)%>%filter(Type=="Nocturnal") %>%drop_na(Transect) %>% group_by(Site, Transect) %>% mutate(NRep = length(unique(Repetition))) %>% select(-Repetition)%>% distinct()
...
nrep_diurnal_distance <- survey_all %>% select(Site, Transect, Repetition, Type)%>%filter(Type=="Diurnal") %>%drop_na(Transect) %>% group_by(Site, Transect) %>% mutate(NRep = length(unique(Repetition))) %>% select(-Repetition)%>% distinct()
...
nrep_cheiro_distance <- survey_all %>%filter(Mois==1|Mois==2|Mois==3|Mois==11|Mois==12) %>%filter(Type=="Nocturnal") %>%select(Site, Transect, Repetition)%>%drop_na(Transect) %>% group_by(Site, Transect) %>% mutate(NRep = length(unique(Repetition))) %>% select(-Repetition)%>% distinct()
...
survey_nocturnal <- left_join(survey_nocturnal, nrep_nocturnal_distance, by =c("Site", "Transect"))
...
survey_nocturnal$Effort.distance <- survey_nocturnal$Length* survey_nocturnal$NRep.y
  
survey_diurnal <- left_join(survey_diurnal, transect_lengths, by =c("Site", "Transect"))
survey_diurnal <- left_join(survey_diurnal, nrep_diurnal_distance, by =c("Site", "Transect"))
...
allocebus_df3$Region.Label <- allocebus_df3$Site_Transect.x
...
ds_allocebus <- ds(allocebus_df3, truncation = 20)
